...
prisons <- prisons %>% filter(grepl(str_list,NAME)) %>% select(NAME,Code,county,POPULATION,CAPACITY) %>% 
...
prison_df <- merge(statenames,prisons) %>% 
  ...
  mutate(prison_fullness = prison_pop/prison_capacity) %>%  # sum over all prisons in a county
...
nyt_with_prisons <- merge(nytdf, prison_counties_frac_released, all.x=TRUE) %>% group_by(countystate) %>%
  ...
  select(countystate,date,cases,prison_county,frac_released,diff_cases,prison_pop,prison_capacity,prison_fullness)
