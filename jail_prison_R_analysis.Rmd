...
```{r}
tmp <- merge(nyt_data,PRDF) %>% group_by(state,date) %>% mutate(total_cases = sum(cases)) %>% select(state,date,total_cases,frac_released) %>% unique()
ggplot(tmp) + geom_line(aes(date,total_cases,color=state))
```
...
state_PRDF <- merge(nyt_data,PRDF) %>% filter(state=='Vermont') %>% select(county,state,date,cases)
state_PRDF
ggplot(state_PRDF) + geom_line(aes(date,cases,color=county))
```

```{r}
PRDF <- merge(pr,prison_df) %>% select(State,frac_released,county) %>% rename(state=State)
nyt_data$county <- tolower(nyt_data$county)
state_PRDF <- merge(nyt_data,PRDF) %>% filter(state=='Pennsylvania') %>% select(county,state,date,cases)
state_PRDF
ggplot(state_PRDF) + geom_line(aes(date,cases,color=county))
```
...
unname(quantile(jp_frac_rel$frac_released, c(.2,.4,.6,.8,1)))
